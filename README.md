...
用 DeepSeek 做中文总结（标题也翻译成中文，摘要约 300 字，上下浮动不超过 5 字），
...
- 若最近 30 小时内有**已结束**的排位赛 / 冲刺赛 / 正赛，按结束时间升序为每场生成一份
  短信友好简报（硬上限 300 字），追加到同一封邮件末尾，不另发邮件。
  （窗口比 24 小时宽 6 小时，是为了容忍 GitHub 定时任务的延迟；已经生成过的 Session
  不会重复发送。）
...
- 推送正文为纯文本（无 URL、无表格）；超过 `sms.max_chars`（默认 1500）时自动裁剪：
